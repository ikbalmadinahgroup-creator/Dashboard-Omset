...
# selalu tampil walau folder assets/ tidak ikut ter-copy ke tempat deploy. Juga dipakai
# sebagai favicon/ikon aplikasi (page_icon) supaya browser/PWA tidak memakai ikon default
# Streamlit (bar chart generik yang muncul dengan tooltip "streamlitApp").
# --------------------------------------------------------------------------------------
...
st.set_page_config(page_title="Dashboard Omset MFlash", layout="wide", page_icon=_page_icon())

# Sembunyikan chrome bawaan Streamlit (menu hamburger, tombol Deploy, status widget,
# footer "Made with Streamlit") supaya tampilannya bersih untuk presentasi ke pihak luar.
st.markdown(
    """
    <style>
...
def _dedup_category_plans(insights: list) -> dict:
    """Ambil satu rencana aksi (online/offline) per kategori yang muncul di daftar insight,
    supaya tidak diulang-ulang untuk tiap cabang (dipakai di slide/halaman 'Rencana Perbaikan')."""
    plans = {}
    for item in insights:
        cat = item.get("category")
        if not cat or cat in plans:
            continue
        plans[cat] = {"online": item.get("online") or [], "offline": item.get("offline") or []}
    return plans
...
def _render_online_offline_html(online: list, offline: list) -> str:
    ...
    if not online and not offline:
        return ""
    html = "<div style='display:flex;gap:20px;flex-wrap:wrap;margin-top:8px;'>"
    if online:
        html += (
            "<div style='flex:1;min-width:230px;'><b>🌐 Online</b>"
            f"<ul style='margin:4px 0 0 18px;padding:0;font-size:12.5px;'>{_bullets(online)}</ul></div>"
        )
    if offline:
        html += (
            "<div style='flex:1;min-width:230px;'><b>🏬 Offline</b>"
            f"<ul style='margin:4px 0 0 18px;padding:0;font-size:12.5px;'>{_bullets(offline)}</ul></div>"
        )
    html += "</div>"
    return html
